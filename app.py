...
WELCOME_EN = (
    "Hi! I am your Web3 Diagnostic AI Agent 👋\n\n"
...
LANG = {
    ...
    "en": {
        ...
        "caps": [
            ...
            "✅ Check ETH balance",
            "✅ Check ERC-20 token history",
            "✅ Diagnose Swap failures",
            "✅ Guide token import",
            "✅ Explain Gas / Slippage",
        ...
        "spinner": "Diagnosing...",
        "welcome": WELCOME_EN,
        "q1": "I sent tokens but not showing in wallet. Tx: 0x5ed67f77ab4d242e81c8b20b55a06e2fb04c2f0b6c0a97fbf1e2614de51d852b7",
        "q2": "My Swap failed. Hash: 0x462407e84d8a3df988a79e1fa1dc68cc8c3a6b9e5413087b7f4c1a9f912089a8",
        "q3": "Check balance for: 0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045",
        ...
        "q6": "MetaMask shows wrong network. How to switch to Ethereum mainnet?",
...
SYSTEM_PROMPT = (
    "You are a senior Web3 Customer Success technical consultant "
    "specializing in diagnosing wallet and token issues. "
    "Capabilities: check transaction status, wallet balance, ERC-20 tokens, diagnose Web3 issues. "
    "Style: concise, professional, plain language, always end with a Next Step. "
...
def check_transaction(tx_hash):
    try:
        ...
        if not result:
            return f"Transaction {tx_hash[:16]}... not found. May be pending or hash incorrect."
        ...
        if status == "0x1":
            ...
            return f"Status: FAILED | Block: {block} | Gas Used: {gas:,}\nReverted. Causes: low gas, high slippage, or insufficient allowance."
...
def check_balance(address):
    try:
        ...
        result = resp.json().get("result")
        if not result:
            return f"Cannot retrieve balance for {address[:16]}..."
        eth = int(result) / 1e18
        return f"Address: {address}\nETH Balance: {eth:.6f} ETH\nNote: Does not include ERC-20 tokens."
    except Exception as e:
        return f"Error: {e}"
...
def check_tokens(address):
    try:
        ...
        lines.append("Import contract address above to see tokens in wallet.")
        return "\n".join(lines)
    except Exception as e:
        return f"Error: {e}"
...
<style>
.header-box {
    background: linear-gradient(135deg, #0A2342, #1B4F8A);
...
with st.sidebar:
    ...
    for key, btn_label in [
        ("q1", L["btn1"]), ("q2", L["btn2"]), ("q3", L["btn3"]),
